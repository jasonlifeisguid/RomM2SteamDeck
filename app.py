...
# Application version — single source of truth (build-appimage.sh reads this line)
APP_VERSION = "1.0.4"
...
def init_database():
    ...
    # Add columns introduced after initial release (for existing databases)
    migration_columns = [
        ("romm_fs_slug", "TEXT DEFAULT ''"),
        ("auto_extract", "INTEGER DEFAULT 0"),
        ("install_paths", "TEXT DEFAULT '[]'"),
    ]
    for column, definition in migration_columns:
        if not db.column_exists("platforms_matching", column):
            db.execute_query(f"ALTER TABLE platforms_matching ADD COLUMN {column} {definition}")
...
def open_default_browser(url):
    ...
    On Linux, xdg-open is used so the default browser (e.g. Librewolf) is
    launched rather than whatever Python's webbrowser module tries first.
    """
...
def maybe_open_browser():
    ...
    try:
        ...
        if should_open:
            open_default_browser(launch_url)
...
def get_romm_base_url():
    ...
    # Remove trailing /api suffix if present to get base URL
    return re.sub(r'/api/?$', '', url) if url else ""
...
# API: Get exe files from an extracted game folder
@app.route('/api/downloads/<int:rom_id>/exe_files')
def api_exe_files(rom_id):
    """Return list of .exe files found inside a downloaded/extracted game folder."""
    try:
        db = get_db()
        rows = db.select_as_dict("downloads", condition="rom_id = ?", condition_values=(rom_id,))
        if not rows:
            return jsonify({"success": False, "error": "Download not found"})
        file_path = rows[0]['file_path']
        if not os.path.isdir(file_path):
            # file_path may point directly to an exe (flat extraction with no subfolder)
            if os.path.isfile(file_path) and file_path.lower().endswith('.exe'):
                name = os.path.basename(file_path)
                return jsonify({"success": True, "exe_files": [
                    {"name": name, "path": file_path, "relative_path": name}
                ]})
            # file_path may point to a non-exe file inside the game dir — scan the parent
        ...
        for root, dirs, files in os.walk(file_path):
            dirs[:] = [d for d in dirs if d.lower() not in ('__macosx', '.git')]
            for f in files:
                if f.lower().endswith('.exe'):
                    full = os.path.join(root, f)
                    rel  = os.path.relpath(full, file_path)
                    exe_files.append({"name": f, "path": full, "relative_path": rel})
        exe_files.sort(key=lambda x: x['relative_path'].count(os.sep))
        return jsonify({"success": True, "exe_files": exe_files})
    except Exception as e:
        system_logger.error(f"Error scanning exe files for rom {rom_id}: {e}")
        return jsonify({"success": False, "error": str(e)}), 500


# API: Platform info (OS detection for frontend feature gating)
@app.route('/api/platform_info')
def api_platform_info():
    """Return the server OS platform string."""
    return jsonify({"success": True, "platform": sys.platform})


# API: Create a desktop shortcut to a selected exe
@app.route('/api/create_shortcut', methods=['POST'])
def api_create_shortcut():
    """Create a desktop shortcut for a Windows/PC game exe.
    Shortcut type depends on the server OS:
      Windows → .lnk (via PowerShell WScript.Shell)
      Linux/Steam Deck → .desktop file
      macOS → .command shell script
    """
    try:
        data      = request.json or {}
        exe_path  = data.get('exe_path', '').strip()
        game_name = data.get('game_name', 'Game').strip()
        if not exe_path or not os.path.isfile(exe_path):
            return jsonify({"success": False, "error": "Executable not found"})
        desktop = os.path.join(os.path.expanduser('~'), 'Desktop')
        os.makedirs(desktop, exist_ok=True)
        # Single line only — newlines in a name would break .desktop entries
        ...
        if sys.platform == 'win32':
            ...
            # Paths go in via env vars — never interpolated into the command
            # string, so path contents can't be executed as PowerShell.
            ps = (
            ...
            env = dict(os.environ,
                       R2SD_SHORTCUT=shortcut_path,
                       R2SD_TARGET=exe_path,
                       R2SD_WORKDIR=working_dir)
            subprocess.run(['powershell', '-NoProfile', '-Command', ps], check=True, env=env)

        elif sys.platform == 'linux':
            shortcut_path = os.path.join(desktop, f"{safe_name}.desktop")
            content = (
                '[Desktop Entry]\n'
                'Type=Application\n'
                f'Name={game_name}\n'
            ...
            shortcut_path = os.path.join(desktop, f"{safe_name}.command")
            with open(shortcut_path, 'w') as fh:
                fh.write(f'#!/bin/bash\nopen "{exe_path}"\n')
            os.chmod(shortcut_path, 0o755)

        else:
            return jsonify({"success": False, "error": f"Unsupported platform: {sys.platform}"})

        return jsonify({"success": True, "shortcut_path": shortcut_path})
    except Exception as e:
        system_logger.error(f"Error creating shortcut: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_delete_download(rom_id):
    ...
    try:
        ...
        # Safety guard: never delete a configured root folder (platform rom
        # folder or install path) even if a bad record points at one
        if file_path:
            db = get_db()
            protected = set()
            for row in db.select_as_dict("platforms_matching", ['steamdeck_platform_name', 'install_paths']):
                folder = (row.get('steamdeck_platform_name') or '').strip()
                if folder:
                    protected.add(os.path.normcase(os.path.normpath(folder)))
                try:
                    for p in json.loads(row.get('install_paths') or '[]'):
                        if p and p.strip():
                            protected.add(os.path.normcase(os.path.normpath(p.strip())))
                except (json.JSONDecodeError, TypeError):
                    pass
            if os.path.normcase(os.path.normpath(file_path)) in protected:
                return jsonify({
                    "success": False,
                    "error": "Refusing to delete: path is a configured platform/install folder"
                }), 400
...
def api_download_progress(rom_id):
    ...
    def generate():
        # Bail out if no progress entry appears for this long (seconds) —
        # otherwise a client polling a dead/unknown rom_id spins forever.
        idle_timeout = 30
        last_seen = time.time()
        ...
        while True:
            ...
            if progress:
                ...
                # If download reached a terminal state, stop streaming
                ...
                yield f"data: {json.dumps({'status': 'error', 'error': 'No active download'})}\n\n"
                break
...
def download_with_extraction_async(romm, rom_id, filename, platform_id, rom_name, file_size, install_path, staging_path='', cancel_event=None):
    ...
    try:
        ...
        if extract_success and (is_zip or is_7z):
            ...
            # Ignore the archive itself if it was downloaded into install_path
            new_items.discard(os.path.basename(downloaded_file))
            if new_items:
                new_paths = [os.path.join(install_path, item) for item in sorted(new_items)]
                # Prefer directories over loose files; a game archive normally
                # extracts to a single top-level folder
                new_dirs = [p for p in new_paths if os.path.isdir(p)]
                game_folder = new_dirs[0] if new_dirs else new_paths[0]
                extract_message = f"Extracted to {game_folder}"
            else:
                # Extraction merged into pre-existing folders and we can't tell
                # which one — never track install_path itself, or a later
                # delete would wipe every game in it
                game_folder = ''
                extract_message = f"Extracted into {install_path} (existing folder, not tracked for deletion)"
...
def api_quit():
    """Gracefully shut down the application process."""
    def _exit():
        time.sleep(0.3)
        os._exit(0)
...
if __name__ == '__main__':
    ...
    port = app_config["server"].get("port", 5001)
    host = app_config["server"].get("host", "0.0.0.0")

    # Try native window via pywebview; fall back to browser if unavailable or if
    # the required GUI toolkit (GTK/Qt) is not present on this system.
    _webview_ok = False
    try:
        import webview
        _webview_ok = True
    ...
    if _webview_ok:
        # Flask runs in a daemon thread; pywebview owns the main thread.
        flask_thread = threading.Thread(
            target=lambda: app.run(debug=False, use_reloader=False, host=host, port=port),
            daemon=True
        )
        flask_thread.start()

        # Give Flask a moment to bind the port before the window loads
        time.sleep(0.8)
        ...
        try:
            ...
            webview.start()
            # Window closed — daemon flask_thread exits with the process
        except Exception as e:
            # GUI toolkit (GTK/Qt) not available on this system — open browser instead
            system_logger.warning(f"pywebview could not start ({e}), opening browser instead")
            maybe_open_browser()
        ...
        # pywebview not installed — run Flask in main thread directly
        maybe_open_browser()
