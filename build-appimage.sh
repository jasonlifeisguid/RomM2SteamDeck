...
# Read version from app.py so there's a single source of truth
APP_VERSION=$(grep -m1 'APP_VERSION = "' app.py | sed 's/.*APP_VERSION = "\([^"]*\)".*/\1/')
...
# Remove bundled OpenSSL so system libraries are used instead.
# PyInstaller bundles the build machine's libssl, which may be older than what
# the Steam Deck's system libcurl requires, causing kde-open/xdg-open to fail.
echo "Removing bundled OpenSSL to avoid system library conflicts..."
find "dist/$APP_NAME" -name "libssl.so*"    -delete
find "dist/$APP_NAME" -name "libcrypto.so*" -delete
